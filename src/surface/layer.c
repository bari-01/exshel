        ...
        uint32_t serial, uint32_t width, uint32_t height)
{
    ...
    if (ls->configured && (ls->width != width || ls->height != height)) {
        log_debug("LayerSurface resize pending: %ux%u -> %ux%u", ls->width,
                ls->height, width, height);
    ...
    if (ls->on_configure) ls->on_configure(ls->notify_data, width, height);
}
...
layer_surface_closed(void *data, struct zwlr_layer_surface_v1 *layer_surface)
{
    ...
    if (ls->on_closed) ls->on_closed(ls->notify_data);
}
...
bool
layer_surface_init(LayerSurface *ls, WaylandContext *ctx, WaylandSurface *s,
        ...
        int32_t exclusive_zone)
{
    ...
    zwlr_layer_surface_v1_add_listener(
            ls->layer_surface, &layer_surface_listener, ls);
    zwlr_layer_surface_v1_set_size(
            ls->layer_surface, ls->width = width, ls->height = height);
    ...
    log_debug("LayerSurface initialized: size=%ux%u", ls->width, ls->height);
    return true;
fail:
