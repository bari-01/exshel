...
static void
popup_configure(void *data, struct xdg_popup *xdg_popup,
        int32_t x, int32_t y, int32_t width, int32_t height)
{
    PopupSurface *ps = data;
    (void)xdg_popup;

    log_debug("popup_configure: x=%d, y=%d, width=%d, height=%d",
            x, y, width, height);
    ...
    if (ps->configured &&
        ...
        log_debug("PopupSurface geometry changed: (%d,%d %ux%u) -> (%d,%d %dx%d)",
        ...
        enum xdg_positioner_gravity gravity)
{
    ...
    ps->xdg_surface = xdg_wm_base_get_xdg_surface(ctx->xdg_wm_base,
            surface->surface);
    ...
    case POPUP_PARENT_XDG:
        ...
        ps->xdg_popup = xdg_surface_get_popup(ps->xdg_surface,
    ...
    log_debug("PopupSurface initialized: pos=(%d,%d) size=%ux%u",
