        ...
        int32_t height, struct wl_array *states)
{
    ...
    if (ws->configured && width > 0 && height > 0 &&
            ((uint32_t)width != ws->width || (uint32_t)height != ws->height)) {
        log_debug("WindowSurface resize pending: %ux%u -> %dx%d", ws->width,
                ws->height, width, height);
        ws->resize_pending = true;
    }
    if (width > 0) ws->width = (uint32_t)width;
    if (height > 0) ws->height = (uint32_t)height;

    if (ws->on_configure)
        ws->on_configure(ws->notify_data, ws->width, ws->height);
}
...
toplevel_close(void *data, struct xdg_toplevel *xdg_toplevel)
{
    ...
    if (ws->on_closed) ws->on_closed(ws->notify_data);
}
...
{
    log_debug("Initializing WindowSurface: %ux%u title=%s app_id=%s", width,
    ...
    if (title) xdg_toplevel_set_title(ws->xdg_toplevel, title);
    if (app_id) xdg_toplevel_set_app_id(ws->xdg_toplevel, app_id);
    ...
    log_debug("WindowSurface initialized: size=%ux%u", ws->width, ws->height);
    return true;
fail:
